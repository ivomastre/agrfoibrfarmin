...
def about_proj(request):
    return render(request, 'mysite/about_proj.html')

def test(request):
    return render(request, 'mysite/test.html')

def show_frame(request, name):
    x = HolyMoly(name)
    aux = json.loads(x)
    print(aux["cidadeNome"])
    return render(request, 'mysite/show_frame.html', {'name' : aux["cidadeNome"], 'tempMax' : aux["tempMax"], 'tempMin' : aux["tempMin"]})

class TempoEle:
    def __init__(self, cidadeNome, tempMin, tendenciaMin, tempMax, tendenciaMax, umidadeMax, umidadeMin, nascerDoSol,porDoSol, manhaTexto, manhaDirecao, manhaVento, tardeTexto, tardeDirecao, tardeVento, noiteTexto,              noiteDirecao, noiteVento):
...
def HolyMoly(body):
    ...
    umidadeMax = quadroDados.find_element_by_id('quadro1_interno_dados_icone_umidademax').find_element_by_tag_name(
        'p').text
    umidadeMin = quadroDados.find_element_by_id('quadro1_interno_dados_icone_umidademin').find_element_by_tag_name(
        'p').text
